...
use cplfs_api::types::{InodeLike, FType, DIRECT_POINTERS};
...
impl InodeRWSupport for EsFS21 {
	fn i_read(&self, 
		...
		let (ranges_to_read, init_block) = self.generate_ranges(bytes_to_read as usize, off as usize);
		...
		for range in ranges_to_read {
			...
			match inode_iter.next() {
				...
				None => return Err(Self::Error::RW("ran out of blocks to read, inode size is incorrect")),
			}
		}
		Ok(bytes_to_read)
	}
	...
	fn i_write(&mut self, 
		...
		if buf.len() < n { return Err(InodeRWError::RW("buffer is smaller than number of bytes to write."))};
		let sb = self.sup_get().unwrap();
		//   exceeding the block pointer limit
		if DIRECT_POINTERS * sb.block_size < off + n { return Err(InodeRWError::OutOfBlocks("not enough blocks can be assigned to inode to write all data."))};
		//   writing at offset would cause a region of empty space in the inode
		if inode.get_size() < off { return Err(InodeRWError::RW("write would make inode data non contiguous."))};
		// if n is zero, no work has to be done
		if n == 0 { return Ok(())}

		// calculate buffer ranges from which to read, plus starting block and initial offset
		let (ranges, skip_blocks, init_off) = self.generate_writing_ranges(n as usize, off as usize);
...
impl EsFS21 {
	...
	fn calculate_bytes_to_read(&self, inode_size: u64, buf_len: u64, off: u64, n: u64) -> Result<u64, InodeRWError>{
		...
		if ovf {return Err(InodeRWError::RW("trying to read beyond inode boundaries"))};
...
impl<'a> InodeBlockMutIter<'a> {
	fn new(fs: &'a mut FSName, ino: &'a mut <FSName as InodeSupport>::Inode, start_block: u64) -> Self {
		InodeBlockMutIter {
			fs: fs,
			ino: ino,
			i: start_block,
...
impl<'a> Iterator for InodeBlockMutIter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		if self.curr_b.block_no != 0 {self.fs.b_put(&self.curr_b).ok()?}; 
		// if we are at DIRECT_POINTERS, there are no more blocks to allocate/return
		if self.i == DIRECT_POINTERS { return None }
...
mod test_with_utils {
    ...
    use cplfs_api::types::{SuperBlock, Buffer, InodeLike, FType, DIRECT_POINTERS};
    use cplfs_api::fs::{FileSysSupport, BlockSupport, InodeSupport, InodeRWSupport};
    ...
    fn test_generate_write_ranges() {
    	...
    	let OFFSET: u64 = 500;
    	let mut calculated_n = 0;
    	let (ranges, skip_blocks, init_off) = fs.generate_writing_ranges(N as usize, OFFSET as usize);
    ...
    fn test_i_read() {
    	...
    	for i in 0..BLOCK_SIZE {block1.serialize_into(&serialized_data[i as usize], i).unwrap();};
    	fs.b_put(&block1).unwrap();
    	let mut block2 = fs.b_get(SUPERBLOCK.datastart + 1).unwrap();
    	for i in 0..73 {block2.serialize_into(&serialized_data[(i + BLOCK_SIZE) as usize], i).unwrap();};
	    ...
	    assert!(fs.i_read(&i1, &mut Buffer::new_zero(20), 2*BLOCK_SIZE, 10).is_err());
	    // read at end of file
	    assert_eq!(fs.i_read(&i1, &mut Buffer::new_zero(20), BLOCK_SIZE + 128, 10).unwrap(),0);
	    // read more than buffer can contain
	    assert_eq!(fs.i_read(&i1, &mut Buffer::new_zero(20), 0, 100).unwrap(),20);
	    // read just before end of file
	    assert_eq!(fs.i_read(&i1, &mut Buffer::new_zero(100), BLOCK_SIZE + 108, 100).unwrap(),20);
    }
    ...
    fn test_i_write() {
    	...
    	assert!(fs.i_write(&mut i1, &mut Buffer::new_zero(10), 1000, 10).is_err());
    	// buffer does not contain as much data as we want to write
    	assert!(fs.i_write(&mut i1, &mut Buffer::new_zero(0), 0, 10).is_err());
    	// writing this many bytes would make inode too large
    	assert!(fs.i_write(&mut i1, &mut Buffer::new_zero(0), 0, BLOCK_SIZE * (DIRECT_POINTERS + 1)).is_err());
