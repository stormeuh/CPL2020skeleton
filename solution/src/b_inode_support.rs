...
use cplfs_api::types::{Inode, DInode, FType, InodeLike, DINODE_SIZE, DIRECT_POINTERS};
...
impl InodeSupport for EsFS2 {
	...
	fn i_free(&mut self, i: u64) -> Result<(), Self::Error> {
		...
		if inode.get_nlink() > 0 {return Ok(())};
		// check if inode hasn't been freed already
		if inode.get_ft() == FType::TFree {return Err(Self::Error::Alloc("trying to free inode which is already free."))};
	...
	fn i_alloc(&mut self, ft: FType) -> Result<u64, Self::Error> {
		...
		while let Some(inode) = inode_iter.next() {
			if inode.get_ft().eq(&FType::TFree) { break };
		}
	...
	fn i_trunc(&mut self, inode: &mut Self::Inode) -> Result<(), Self::Error> {
		...
		if !inode_block.i_get(i)?.eq(inode) {return Err(Self::Error::Trunc("given inode contents do not correspond to what is on disk!"))};
...
impl EsFS2 {
	...
	fn init_inode_region(&mut self) -> Result<(), <Self as FileSysSupport>::Error> {
		...
		for i in 0..(self.sup_get()?.ninodes) {
			let inode = <Inode as InodeLike>::new(
				i, 
				&FType::TFree,
				0,
				0,
			...
			if inode.is_none() { return Err(InodeError::Init("can't init inode region")) }
	...
	fn deallocate_valid_blocks(&mut self, size: u64, bs: &[u64]) -> Result<(), <Self as FileSysSupport>::Error> {
...
impl InodeBlock {
	...
	fn new_from_inode_i(i: u64, fs: &FSName) -> Result<Self, InodeError> {
		...
		if i >= sb.ninodes && i != 0 {return Err(InodeError::Bounds())};
	...
	fn calc_off(&self, i: u64) -> Result<u64, InodeError> {
		...
		if i > self.ih || ovf { return Err(InodeError::Bounds()) };
		// calculate the byte offset in the block
...
mod test_with_utils {
	...
	use cplfs_api::types::{SuperBlock, InodeLike, FType, DINODE_SIZE, DInode};
	use cplfs_api::fs::{FileSysSupport, BlockSupport, InodeSupport};
