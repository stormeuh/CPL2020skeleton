...
use cplfs_api::types::{DInode, FType, Inode, InodeLike, DIRECT_POINTERS};
use cplfs_api::types::{DirEntry, DIRENTRY_SIZE, DIRNAME_SIZE};
...
impl DirectorySupport for EsFS3 {
    ...
    fn dirlookup(
        ...
        if inode.get_ft() != FType::TDir {
            return Err(DirectoryError::IllegalInode("not a directory"));
        }
        // check the given inode is consistent with the one on disk
        if !self.i_get(inode.get_inum())?.eq(inode) {
            return Err(DirectoryError::IllegalInode("inconsistency on disk"));
        }
    ...
    fn dirlink(
        ...
        if self.i_get(inum)?.get_ft() == FType::TFree {
            return Err(DirectoryError::IllegalDirEntry(
                "Inode referenced by inum is not in use",
            ));
        };
...
impl<'a> DirEntryIterator<'a> {
    ...
    pub fn new(
        ...
        if inode.get_ft() != FType::TDir {
            return Err(DirectoryError::IllegalInode(
                "not a directory, cannot iterate over it",
            ));
        };
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.d_cache.pop_front() {
            ...
            None => {
                ...
                let bytes_read = self
                    .fs
                    .i_read(self.inode, &mut buffer, offset, bytes_to_read)
                    .unwrap();
                ...
                for direntry_i in 0..direntries_read {
                    let direntry = buffer
                        .deserialize_from::<DirEntry>(direntry_i * *DIRENTRY_SIZE)
                        .unwrap();
...
impl FileSysSupport for EsFS3 {
    ...
    fn mkfs<P: AsRef<Path>>(path: P, sb: &SuperBlock) -> Result<Self, Self::Error> {
        ...
        let mut new_fs = EsFS3 { ifs: inode_fs };
...
mod test_with_utils {
    ...
    use cplfs_api::fs::{
        BlockSupport, DirectorySupport, FileSysSupport, InodeRWSupport, InodeSupport,
    };
    use cplfs_api::types::{Buffer, FType, InodeLike, SuperBlock, DIRENTRY_SIZE};
    ...
    fn test_dirlookup() {
        ...
        fs.i_write(&mut iroot, &buffer, offset, *DIRENTRY_SIZE)
            .unwrap();
